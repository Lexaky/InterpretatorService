...
namespace HelloWorldApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                VariableTracker.TrackVariables(1, ("x", x), ("arr", arr), ("matrix", matrix));
                Console.WriteLine(i);
            }
            Console.WriteLine("Step 1");
            Console.WriteLine("Step 2");
            VariableTracker.TrackVariables(2, ("x", x), ("arr", arr));
        }
    }
}
...
public static class VariableTracker
{
    ...
    public static void TrackVariables(int methodId, params (string Name, object Value)[] variables)
    {
        ...
        System.IO.File.AppendAllLines($"{Path.Combine("/app/code_files/", $"{codeId}values.txt")}", lines);
    }
}
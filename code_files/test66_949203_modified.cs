...
class Program
{
    ...
    {
    await TestVariableTracker.Initialize("66_949203", "/app/codeData/66_949203_userdata.txt");
    ...
    public static async Task<(Dictionary<string, object> UpdatedValues, string Error)> TrackVariables(int methodId, int lineNumber, params (string Name, object Value)[] variables)
    {
        try
        {
            ...
            foreach (var (name, value) in variables)
            {
                ...
                {
                    Array arrValue when arrValue.Rank == 1 => string.Join(",", arrValue.Cast<object>().Select(x => x?.ToString() ?? "null")),
                    Array arrValue when arrValue.Rank == 2 => string.Join(";", Enumerable.Range(0, arrValue.GetLength(0)).Select(i => string.Join(",", Enumerable.Range(0, arrValue.GetLength(1)).Select(j => arr.GetValue(i, j)?.ToString() ?? "null")))),
                    _ => value?.ToString() ?? "null"
                };
            ...
            var valuesPath = $"//app/codeData/{_codeId}values.txt";
            var mismatchesPath = $"//app/codeData/{_codeId}mismatches.txt";

...
class Program
{
    ...
    {
    TestVariableTracker.Initialize("5_4", "/app/code_files/5_4_userdata.txt");
        int[] arr = { 5,4,3,2,1 };
        ...
        for (int i = 0; i < arr.Length - 1; i++)
        {
            for (int j = 0; j < arr.Length - i - 1; j++)
            {
                if (arr[j] > arr[j + 1])
                {
                    ...
                    arr[j] = arr[j + 1];
                    arr[j + 1] = temp;
    var updates_18_0 = TestVariableTracker.TrackVariables(18, 1, ("arr", arr), ("i", i), ("j", j));
    if (updates_18_0.ContainsKey("arr")) arr = (int[])updates_18_0["arr"];
    if (updates_18_0.ContainsKey("i")) i = (int)updates_18_0["i"];
    if (updates_18_0.ContainsKey("j")) j = (int)updates_18_0["j"];
                }
            }
    var updates_20_1 = TestVariableTracker.TrackVariables(20, 2, ("arr", arr));

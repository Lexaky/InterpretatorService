...
class Program
{
    ...
    {
    await TestVariableTracker.Initialize("67_294075", "/app/codeData/67_294075_userdata.txt");
        ...
        foreach (int number in numbers)
        {
            ...
            if (updates_12_0.UpdatedValues.ContainsKey("sum")) sum = (int)updates_12_0.UpdatedValues["sum"];
        }
...
public static class TestVariableTracker
{
    ...
    public static async Task Initialize(string codeId, string userDataPath)
    {
        ...
        await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TestVariableTracker] Initialized: codeId={codeId}, userDataPath={userDataPath}\n");
    }
    ...
    public static async Task<(Dictionary<string, object> UpdatedValues, string Error)> TrackVariables(int methodId, int lineNumber, params (string Name, object Value)[] variables)
    {
        try
        {
            ...
            await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] Started: stepNumber={stepNumber}, methodId={methodId}, lineNumber={lineNumber}, variables={string.Join(",", variables.Select(v => v.Name))}\n");
            ...
            {
                await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] User data file not found: {_userDataPath}\n");
                return (updatedValues, $"User data file not found: {_userDataPath}");
            }
            ...
            await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] User values: {string.Join(",", userValues.Select(kv => $"step={kv.StepNumber},tracker={kv.TrackerHitId},{kv.VariableName}={kv.Value}"))}\n");
            ...
            foreach (var (name, value) in variables)
            {
                if (value is Array arr && arr.Rank == 2)
                {
                    ...
                    {
                        string rowValue = string.Join(",", Enumerable.Range(0, arr.GetLength(1)).Select(j => arr.GetValue(i, j)?.ToString() ?? "null"));
                        valuesLines.Add($"{stepNumber}//{methodId}//{name}//{typeName}//2//{rowValue}//{lineNumber}");
                    }
                }
                else
                {
                    ...
                    valuesLines.Add($"{stepNumber}//{methodId}//{name}//{typeName}//{rank}//{stringValue}//{lineNumber}");
                }
            }
            ...
            foreach (var (name, value) in variables)
            {
                ...
                {
                    Array arrValue when arrValue.Rank == 1 => string.Join(",", arrValue.Cast<object>().Select(x => x?.ToString() ?? "null")),
                    Array arrValue when arrValue.Rank == 2 => string.Join(";", Enumerable.Range(0, arrValue.GetLength(0)).Select(i => string.Join(",", Enumerable.Range(0, arrValue.GetLength(1)).Select(j => arrValue.GetValue(i, j)?.ToString() ?? "null")))),
                    _ => value?.ToString() ?? "null"
                };
            ...
            try
            {
                ...
                var directory = Path.GetDirectoryName(valuesPath);
                if (!Directory.Exists(directory))
                {
                    Directory.CreateDirectory(directory);
                    await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] Created directory: {directory}\n");
                }

                if (!File.Exists(valuesPath))
                {
                    await File.WriteAllTextAsync(valuesPath, "");
                    await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] Created values file: {valuesPath}\n");
                }
                if (!File.Exists(mismatchesPath))
                {
                    await File.WriteAllTextAsync(mismatchesPath, "");
                    await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] Created mismatches file: {mismatchesPath}\n");
                }
            ...
            {
                await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] Error writing to files: valuesPath={valuesPath}, mismatchesPath={mismatchesPath}, error={ex.Message}\n");
            }

            await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] Completed: stepNumber={stepNumber}, methodId={methodId}, lineNumber={lineNumber}, valuesCount={valuesLines.Count}, mismatchesCount={mismatchesLines.Count}, valuesPath={valuesPath}, mismatchesPath={mismatchesPath}\n");

            return (updatedValues, error);
        }
        catch (Exception ex)
        {
            await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][TrackVariables] Error: methodId={methodId}, lineNumber={lineNumber}, message={ex.Message}, stackTrace={ex.StackTrace}\n");
    ...
    public static async Task WriteRemainingUserValues(int methodId, int lineNumber)
    {
        try
        {
            if (!File.Exists(_userDataPath))
            {
                await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][WriteRemainingUserValues] User data file not found: {_userDataPath}\n");
                return;
            }
            ...
            if (mismatchesLines.Any())
            {
                ...
                {
                    // Проверяем и создаём директорию
                    var directory = Path.GetDirectoryName(mismatchesPath);
                    if (!Directory.Exists(directory))
                    {
                        Directory.CreateDirectory(directory);
                        await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][WriteRemainingUserValues] Created directory: {directory}\n");
                    }

                    if (!File.Exists(mismatchesPath))
                    {
                        await File.WriteAllTextAsync(mismatchesPath, "");
                        await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][WriteRemainingUserValues] Created mismatches file: {mismatchesPath}\n");
                    }
                    await File.AppendAllLinesAsync(mismatchesPath, mismatchesLines);
                }
                catch (Exception ex)
                {
                    await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][WriteRemainingUserValues] Error writing to {mismatchesPath}: {ex.Message}\n");
                }

                await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][WriteRemainingUserValues] Added {mismatchesLines.Count} remaining user values to mismatches: methodId={methodId}, lineNumber={lineNumber}\n");
            }
        }
        catch (Exception ex)
        {
            await File.AppendAllTextAsync("/app/code_files/debug.txt", $"[{DateTime.Now}][WriteRemainingUserValues] Error: methodId={methodId}, lineNumber={lineNumber}, message={ex.Message}, stackTrace={ex.StackTrace}\n");
        }
    }

        ...
        for (int i = 0; i < arr.Length - 1; i++)
        {
            for (int j = 0; j < arr.Length - i - 1; j++)
            {
                if (arr[j] > arr[j + 1])
                {
                    ...
                    arr[j] = arr[j + 1];
                    arr[j + 1] = temp;
                    var updates = TestVariableTracker.TrackVariables(3, ("arr", arr), ("j", j), ("i", i));
                    if (updates.ContainsKey("arr")) arr = updates["arr"];
                    if (updates.ContainsKey("j")) j = updates["j"];
                    if (updates.ContainsKey("i")) i = updates["i"];
                }
                var updates = TestVariableTracker.TrackVariables(2, ("arr", arr), ("i", i));
                if (updates.ContainsKey("arr")) arr = updates["arr"];
                if (updates.ContainsKey("i")) i = updates["i"];
            }
            var updates = TestVariableTracker.TrackVariables(1, ("arr", arr));
            if (updates.ContainsKey("arr")) arr = updates["arr"];
        }
...
public static class TestVariableTracker
{
    ...
    private static string _codeId = "35_854243";
    ...
    public static void Initialize(string codeId, string userDataFile)
    {
        ...
        {
            var lines = File.ReadAllLines(userDataFile).Skip(1);
    ...
    public static Dictionary<string, object> TrackVariables(int methodId, params (string Name, object Value)[] variables)
    {
        ...
        File.AppendAllLines(Path.Combine("/app/code_files/", $"{_codeId}values.txt"), lines);
        if (_mismatches.Any())
            File.AppendAllLines(Path.Combine("/app/code_files/", $"{_codeId}mismatches.txt"), _mismatches);
